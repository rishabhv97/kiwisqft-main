...
import { Filter, SlidersHorizontal, MapPin, ChevronDown, ChevronUp, Check, Calculator, Save, Download, Layers, Building, Compass, FileText, Info } from 'lucide-react';
import { Loader2 } from 'lucide-react';

interface ListingsProps {
  type: 'sale' | 'rent'; // Changed to match your App.tsx usage
}

const Listings: React.FC<ListingsProps> = ({ type }) => {
  // --- 1. New State for Data Fetching ---
  ...
  // --- 2. Data Fetching Logic (Connected to your Node Backend) ---
  useEffect(() => {
    const fetchProperties = async () => {
        ...
        try {
            ...
            if (data) {
                // Map Database Fields (snake_case) to Frontend (camelCase)
                const mapped: Property[] = data.map((p: any) => ({
                    ...
                    city: p.city,
                    type: p.type,
                    listingType: p.listing_type, // Critical mapping
                    images: typeof p.images === 'string' 
                        ? JSON.parse(p.images).map((img: string) => `http://localhost:5000${img}`) // Add localhost prefix
                        : [],
                    bedrooms: p.bedrooms,
                    ...
                    additionalRooms: typeof p.additional_rooms === 'string' ? JSON.parse(p.additional_rooms) : [],
                    ...
                    facing: p.facing_entry,     // Entry Facing
                    exitFacing: p.facing_exit,  // Exit Facing
                    ...
                    // Missing in some DB rows? Default to false/null
  ...
  const filteredProperties = useMemo(() => {
    return properties.filter(p => {
      ...
      if (selectedAmenities.length > 0) {
        if (!p.amenities) return false; // Safety check
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500">
             {loading ? 'Loading properties...' : `${filteredProperties.length} listings found matching your criteria`}
          </p>
        </div>
          ...
          <div className="lg:w-3/4">
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[1,2,3,4].map(n => (
                        <div key={n} className="h-96 bg-gray-100 rounded-xl animate-pulse flex items-center justify-center text-gray-400">
